...
- **이미지 체크 모드**: 11:05~11:11 모든 코너 이미지 업로드 대기, 미완료 시 전송 안 함
- **이미지 관리**: 날짜별 파일명(`course_0_20260330.jpg`) + 전송 전 기존 이미지 cleanup
- **자동 스케줄링**: Google Cloud Run + Cloud Scheduler로 평일 11:05 KST 자동 실행

## 아키텍처

```
Cloud Scheduler (평일 11:05 KST)
  └─ Cloud Run Job (서울 리전, --check 모드)
       ├─ 60초 간격으로 이미지 업로드 상태 체크 (6/6 대기)
       ├─ 모든 이미지 확인 시:
       │    ├─ 이미지 다운로드
       │    ├─ GitHub Pages 기존 이미지 cleanup
       │    ├─ 새 이미지 push + Pages 배포 대기
       │    └─ Slack Webhook 전송
       └─ 11:11까지 미완료 시 전송 건너뜀
```
...
# 이미지 체크 모드 (Cloud Run용): 6/6 이미지 대기 후 전송
